...
class MoyenneGlissante(Transformation):
    ...
    def transforme(self):
        ...
        for i in range(1,len(self.table.lignes)+1):
            ...
            if convertirdate(self.table.lignes[i][j]) - timedelta((self.periode-1)/2) < convertirdate(self.table.lignes[0][j]) or convertirdate(self.table.lignes[i][j]) + timedelta((self.periode-1)/2) > convertirdate(self.table.lignes[-1][j]):
                mg.insert(i,None)
            else:
